...
def notify_n8n_task_assigned(task: dict, assignee_row: sqlite3.Row, assigner_row: sqlite3.Row) -> None:
    ...
    threading.Thread(target=_post_webhook, args=(N8N_WEBHOOK_URL, payload), daemon=True).start()
...
def init_db() -> None:
    ...
    # Migration for DBs created before is_active existed.
    try:
    ...
    if existing == 0:
        ...
        for idx, (name, email, initials, color) in enumerate(ROSTER, start=1):
            ...
            rows.append((
                f"u{idx}", name, initials, role,
                email_l, hash_pw(_default_password(name)), color,
            ))
        db.executemany(
            "INSERT INTO users (id,name,initials,role,email,password,color) VALUES (?,?,?,?,?,?,?)",
            rows,
        )
        db.commit()
        print(f"✅ Seeded {len(rows)} users. Default password pattern: <FirstName>@2026")
    db.close()
...
def login():
    ...
    if not user or user["password"] != hash_pw(password):
        # Generic message — never leak which of the two was wrong.
...
def get_users():
    rows = query(
        "SELECT id,name,initials,role,color,email FROM users WHERE is_active=1 ORDER BY "
        "CASE role WHEN 'admin' THEN 0 ELSE 1 END, name"
    )
    out = []
    for u in rows:
        stats = query("SELECT status FROM tasks WHERE assigned_to=?", (u["id"],))
        total  = len(stats)
...
def admin_add_user():
    ...
    new_id = f"u{count+1}"
    while query("SELECT id FROM users WHERE id=?", (new_id,), one=True):
        count += 1
        new_id = f"u{count+1}"
    ...
    user = query("SELECT * FROM users WHERE id=?", (new_id,), one=True)
...
def admin_remove_user(user_id):
    ...
    mutate("UPDATE users SET is_active=0 WHERE id=?", (user_id,))
    log_activity(
...
def admin_restore_user(user_id):
    ...
    mutate("UPDATE users SET is_active=1 WHERE id=?", (user_id,))
    log_activity(
...
def _serialize_task(t: sqlite3.Row, updates: list) -> dict:
    return {
...
def get_tasks():
    ...
    if role == "admin":
        rows = query(_TASK_SELECT + " ORDER BY t.created_at DESC, t.rowid DESC")
    else:
        rows = query(_TASK_SELECT + " WHERE t.assigned_to = ? ORDER BY t.created_at DESC, t.rowid DESC", (uid,))
...
def create_task():
    ...
    user = query("SELECT * FROM users WHERE id=?", (assignee,), one=True)
    if not user:
    ...
    while query("SELECT id FROM tasks WHERE id=?", (task_id,), one=True):
        count += 1
        task_id = f"Task-{count+1:03d}"
    ...
    task    = query(_TASK_SELECT + " WHERE t.id=?", (task_id,), one=True)
    updates = query("""
        SELECT tu.*, u.name AS updater_name FROM task_updates tu
        JOIN users u ON tu.updated_by = u.id
        WHERE tu.task_id = ? ORDER BY tu.created_at DESC, tu.id DESC
    """, (task_id,))
    serialized = _serialize_task(task, updates)

    assigner_row = query("SELECT * FROM users WHERE id=?", (session["user_id"],), one=True)
...
def delete_task(task_id):
    ...
    mutate("DELETE FROM task_updates WHERE task_id=?", (task_id,))
    mutate("DELETE FROM tasks WHERE id=?", (task_id,))
    log_activity(
...
def update_task(task_id):
    ...
    mutate("UPDATE tasks SET status=? WHERE id=?", (new_status, task_id))
    mutate(
        "INSERT INTO task_updates (task_id,updated_by,message,old_status,new_status) VALUES (?,?,?,?,?)",
...
def log_activity(icon, color, icon_color, text, via_wa=0):
    mutate(
        "INSERT INTO activity_log (icon,color,icon_color,text,via_wa) VALUES (?,?,?,?,?)",
...
def get_stats():
    ...
    if role == "admin":
        ...
        rows = query("SELECT status, due_date FROM tasks WHERE assigned_to=?", (uid,))
...
def get_performance():
    ...
    for m in members:
        ...
        assigned = query(
            "SELECT * FROM tasks WHERE assigned_to=? AND date(created_at)>=?",
            (mid, since),
        )
        total     = len(assigned)
        ...
        for t in assigned:
            ...
            completion = query(
                ...
                "WHERE task_id=? AND new_status='completed' "
        ...
        updates      = query(
            "SELECT * FROM task_updates WHERE updated_by=? AND date(created_at)>=?",
            (mid, since),
        )
        ...
        for t in assigned:
            first = query(
                "SELECT created_at FROM task_updates "
                "WHERE task_id=? AND updated_by=? ORDER BY created_at ASC, id ASC LIMIT 1",
                (t["id"], mid), one=True,
            )
        ...
        daily = {}
        for t in query("SELECT * FROM tasks WHERE assigned_to=?", (mid,)):
...
def change_password():
    ...
    mutate("UPDATE users SET password=? WHERE id=?", (hash_pw(new_pw), session["user_id"]))
...
def admin_reset_password(user_id):
    ...
    mutate("UPDATE users SET password=? WHERE id=?", (hash_pw(new_pw), user_id))
